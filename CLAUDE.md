...
- [ ] このドキュメントを読み、関連するルールを理解した
- [ ] ディレクトリ構造に従ってファイルを配置する
- [ ] 命名規則に従っている
- [ ] 型定義を適切に行っている
- [ ] エラーハンドリングを実装している
- [ ] レスポンシブデザインを考慮している
- [ ] shadcn/uiコンポーネントを優先的に使用している
- [ ] 環境変数を適切に管理している
- [ ] パフォーマンスを考慮している
- [ ] セキュリティを考慮している
...
- [ ] 複雑なロジックはカスタムフックに分離されているか
- [ ] データベース更新はサーバーアクションを使用しているか
- [ ] バリデーションはクライアント・サーバー両方で実装しているか
- [ ] 定数は constants.ts で管理しているか
- [ ] 型定義は適切に配置されているか (types/)
- [ ] ファイル命名規則に従っているか
- [ ] エラーハンドリングは適切か
- [ ] レスポンシブデザインに対応しているか
...
疑問がある場合は、このドキュメントを更新し、チーム全体で共有してください。
